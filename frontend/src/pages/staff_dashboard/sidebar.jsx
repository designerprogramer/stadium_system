...
const navItems = [
  ...
  { to: "ticket-checking", icon: QrCode, label: "Scanner" },
  { to: "non-buy-tickets", icon: Ticket, label: "Manual Tickets" },
...
function iconClasses(isActive) {
  return isActive
    ? "group relative flex h-11 w-11 items-center justify-center rounded-xl bg-sky-50 text-sky-600"
    : "group relative flex h-11 w-11 items-center justify-center rounded-xl text-slate-500 transition hover:bg-sky-50 hover:text-sky-600";
}
...
export default function StaffSidebar() {
  return (
    <aside className="flex h-screen w-20 flex-col items-center justify-between border-r border-slate-200 bg-white/95 py-6 backdrop-blur-xl">
      <div className="flex flex-col items-center">
        ...
        <nav className="mt-8 flex flex-col gap-3">
          {navItems.map((item) => {
            ...
            return (
              <NavLink key={item.to} to={item.to} className={({ isActive }) => iconClasses(isActive)}>
